...
export function ComparisonChart({ pools, selectedAddresses, onClose }: ComparisonChartProps) {
  ...
  // CRITICAL: Memoize selected pools with stable reference
  const selectedPools = useMemo(() =>
          pools.filter(p => selectedAddresses.includes(p.address)),
      [pools, selectedAddresses]
  );

  // CRITICAL: Create stable pool keys for consistent chart rendering
  const poolKeys = useMemo(() =>
          selectedPools.map(pool => `${pool.token0}/${pool.token1}`),
      [selectedPools]
  );

  // FIX: Generate completely new chart data when selected pools change
  useEffect(() => {
    ...
    const generateHistoricalData = (): ChartDataPoint[] => {
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        // CRITICAL FIX: Create completely new object with no shared references
        const point: ChartDataPoint = {
          time: time.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }),
        };

        // CRITICAL FIX: Generate data independently for each pool
        selectedPools.forEach((pool) => {
          const key = `${pool.token0}/${pool.token1}`;
          const baseApy = pool.apy;
          const variation = (Math.random() - 0.5) * 2;

          // Store as completely new value, not reference
    ...
    // CRITICAL FIX: Always regenerate from scratch when pools change
    ...
    const interval = setInterval(() => {
      setChartData(prevData => {
        ...
        // CRITICAL FIX: Create completely new point object
        const newPoint: ChartDataPoint = {
          time: now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }),
        };

        // CRITICAL FIX: Copy current APY values (not references)
        selectedPools.forEach(pool => {
          const key = `${pool.token0}/${pool.token1}`;
          // Use the current APY value directly from the pool
          newPoint[key] = parseFloat(pool.apy.toFixed(2));
        });

        // CRITICAL FIX: Create new array (immutable update)
        // Keep last 29 points + new point = 30 total
        return [...prevData.slice(1), newPoint];
      });
    }, 3000);

    return () => clearInterval(interval);
  }, [selectedPools]); // CRITICAL FIX: Depend on selectedPools, not individual pools
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              {selectedPools.map((pool, index) => {
                const dataKey = `${pool.token0}/${pool.token1}`;
                return (
                    <Line
                        key={`${pool.address}-${dataKey}`} // CRITICAL FIX: Stable unique key
                        type="monotone"
                        dataKey={dataKey}
                        stroke={COLORS[index]}
                        strokeWidth={2}
